...
import { PlusCircle, Search, Filter, FileDown, UserCheck } from 'lucide-react';
import { useToast } from "@/hooks/use-toast";
import { allAuditCategories, allAuditStatuses, allAppRoles, allMockComplianceChapters } from '@/lib/mock-data';
import { parseISO } from 'date-fns';

// Specific form data type for this page, matching AuditRecordFormDialog's expectation
type MovementLogFormData = Omit<AuditRecord, 'id' | 'createdAt' | 'updatedAt' | 'category' | 'lastCompletedDate'> & {
  category?: AuditToolCategory; 
};

export default function MovingInOutPage() {
  const [movementRecords, setMovementRecords] = useState<AuditRecord[]>([]);
  ...
  }>({ status: 'all', category: 'Resident Admissions & Discharges', chapterReferenceTag: 'all', assignedRole: 'all' });
  ...
  useEffect(() => {
    const fetchMovementRecords = async () => {
      setLoading(true);
      setError(null);
      try {
        const response = await fetch('/api/admissions-discharges');
        if (!response.ok) {
          throw new Error(`Failed to fetch admission/discharge records: ${response.statusText}`);
        }
        ...
        setMovementRecords(processedData);
      } catch (err: any) {
        console.error("Error fetching admission/discharge records:", err);
        setError("Could not load records. Please try again later.");
        toast({ variant: "destructive", title: "Error", description: "Could not load admission/discharge records." });
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredMovementRecords = useMemo(() => {
    return movementRecords.filter(record => {
  ...
  }, [movementRecords, searchTerm, filters]);
  ...
  const handleSaveRecord = (formData: MovementLogFormData) => {
    const recordCategory = 'Resident Admissions & Discharges'; 
    ...
    if (editingRecord) {
      const updatedRecord: AuditRecord = { 
        ...
        updatedAt: new Date(),
        lastCompletedDate: (formData.status === 'Admission Complete' || formData.status === 'Discharge Complete' || formData.status === 'Resolved') ? new Date() : editingRecord.lastCompletedDate,
       };
      setMovementRecords(prev => prev.map(r => r.id === editingRecord.id ? updatedRecord : r));
      toast({ title: "Movement Log Updated", description: `Log entry "${updatedRecord.name}" saved.` });
    } else {
      const newRecord: AuditRecord = {
        ...
        createdAt: new Date(),
        updatedAt: new Date(),
        lastCompletedDate: (formData.status === 'Admission Complete' || formData.status === 'Discharge Complete' || formData.status === 'Resolved') ? new Date() : null,
      };
      setMovementRecords(prev => [newRecord, ...prev]);
      toast({ title: "Movement Log Added", description: `New log entry "${newRecord.name}" added.` });
    }
    handleCloseForm();
  };
  ...
  const handleSaveEvidence = (recordId: string, evidenceLink: string) => {
    setMovementRecords(prev => prev.map(r => r.id === recordId ? { ...r, evidenceLink, updatedAt: new Date() } : r));
    ...
    toast({ title: "Evidence Link Saved", description: `Evidence for log entry updated.` });
    handleCloseEvidenceModal();
  };

  const handleExportData = () => {
    if (filteredMovementRecords.length === 0) {
      toast({ title: "No Data to Export", description: "Please refine your filters or add data.", variant: "default" });
      return;
    }
    const headers = ["ID", "Log Entry Name/Resident", "Category", "Staff Involved", "Validator", "Event Date", "Status", "Document Link", "Chapter Reference", "Notes", "Created At", "Updated At"];
    const rows = filteredMovementRecords.map(record => [
      record.id,
      record.name,
      record.category,
      Array.isArray(record.assignedRole) ? record.assignedRole.join('; ') : record.assignedRole,
      record.validator || '',
      record.lastCompletedDate ? record.lastCompletedDate.toISOString().split('T')[0] : '', // Using lastCompletedDate as the primary date for the event
      record.status,
      record.evidenceLink || '',
    ...
    link.setAttribute("download", "resident_movement_log_export.csv");
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    toast({ title: "Export Successful", description: "Resident movement log data exported as CSV." });
  };

  const movementSpecificStatuses: AuditStatus[] = ['Admission Pending', 'Admission Complete', 'Discharge Pending', 'Discharge Complete', 'Archived'];
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Resident Admissions & Discharges Log
          </CardTitle>
          <CardDescription>
            Track and manage resident admissions, discharges, and related documentation.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="mb-6 p-4 bg-muted/30 rounded-lg shadow space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-end">
              ...
              <Select value={String(filters.status)} onValueChange={(value) => handleFilterChange('status', value)}>
                ...
                <SelectContent>
                  ...
                  {movementSpecificStatuses.map(stat => <SelectItem key={stat} value={stat}>{stat}</SelectItem>)}
                </SelectContent>
              </Select>
              <Select value={String(filters.assignedRole)} onValueChange={(value) => handleFilterChange('assignedRole', value)}>
                <SelectTrigger>
                  <Filter className="mr-2 h-4 w-4 text-muted-foreground" />
                  <SelectValue placeholder="Filter by Staff Involved" />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex justify-end space-x-2 pt-2">
                <Button onClick={() => handleOpenForm()}>
                    <PlusCircle className="mr-2 h-5 w-5" /> Log New Admission/Discharge
                </Button>
          ...
          <AuditTable 
            records={filteredMovementRecords}
