...
import { PlusCircle, Search, Filter, FileDown, Users2 } from 'lucide-react';
...
export default function CaseManagementPage() {
  const [caseRecords, setCaseRecords] = useState<AuditRecord[]>([]);
  ...
  }>({ status: 'all', category: 'Case Management Coordination', chapterReferenceTag: 'all', assignedRole: 'all' });
  ...
  useEffect(() => {
    const fetchCaseRecords = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch case management records: ${response.statusText}`);
        }
        ...
        setCaseRecords(processedData);
      } catch (err: any) {
        console.error("Error fetching case management records:", err);
        setError("Could not load case management records. Please try again later.");
        toast({ variant: "destructive", title: "Error", description: "Could not load case management records." });
      } finally {
        setLoading(false);
      }
    };
  ...
  }, [caseRecords, searchTerm, filters]);
  ...
  const handleSaveRecord = (formData: AuditRecordFormData) => {
    const recordCategory = 'Case Management Coordination'; 
    ...
    if (editingRecord) {
      const updatedRecord: AuditRecord = { 
        ...
        updatedAt: new Date(),
        lastCompletedDate: (formData.status === 'Resolved' || formData.status === 'Active Engagement') ? new Date() : editingRecord.lastCompletedDate,
       };
      setCaseRecords(prev => prev.map(r => r.id === editingRecord.id ? updatedRecord : r));
      toast({ title: "Case Record Updated", description: `Record "${updatedRecord.name}" saved.` });
    } else {
      const newRecord: AuditRecord = {
        id: `casemgmt_${Date.now()}`, 
        name: formData.name,
        category: recordCategory, 
        ...
        createdAt: new Date(),
        updatedAt: new Date(),
        lastCompletedDate: (formData.status === 'Resolved' || formData.status === 'Active Engagement') ? new Date() : null,
      };
      setCaseRecords(prev => [newRecord, ...prev]);
      toast({ title: "Case Record Added", description: `New record "${newRecord.name}" added.` });
    }
    handleCloseForm();
  };
  ...
  const handleSaveEvidence = (recordId: string, evidenceLink: string) => {
    setCaseRecords(prev => prev.map(r => r.id === recordId ? { ...r, evidenceLink, updatedAt: new Date() } : r));
    ...
    toast({ title: "Evidence Link Saved", description: `Evidence for case record updated.` });
    handleCloseEvidenceModal();
  };

  const handleExportData = () => {
    if (filteredCaseRecords.length === 0) {
      toast({ title: "No Data to Export", description: "Please refine your filters or add data.", variant: "default" });
      return;
    }
    const headers = ["ID", "Record Name/Subject", "Category", "Assigned Staff", "Validator", "Last Action Date", "Status", "Document Link", "Chapter Reference", "Notes", "Created At", "Updated At"];
    const rows = filteredCaseRecords.map(record => [
      record.id,
      record.name,
      record.category,
    ...
    link.setAttribute("download", "case_management_export.csv");
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    toast({ title: "Export Successful", description: "Case management data exported as CSV." });
  };

  const caseManagementSpecificStatuses: AuditStatus[] = ['Active Engagement', 'Referral Made', 'Follow-up Scheduled', 'Action Required', 'Resolved', 'Archived'];
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Case Management Agencies & Coordination
          </CardTitle>
          <CardDescription>
            Manage records related to external case management agencies and resident coordination.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="mb-6 p-4 bg-muted/30 rounded-lg shadow space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-end">
              ...
              <Select value={String(filters.status)} onValueChange={(value) => handleFilterChange('status', value)}>
                ...
                <SelectContent>
                  ...
                  {caseManagementSpecificStatuses.map(stat => <SelectItem key={stat} value={stat}>{stat}</SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="flex justify-end space-x-2 pt-2">
                <Button onClick={() => handleOpenForm()}>
                    <PlusCircle className="mr-2 h-5 w-5" /> Add New Case Record
                </Button>
