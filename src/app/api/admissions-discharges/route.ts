...
import { mockAdmissionDischargeRecords } from '@/lib/mock-data';
...
export async function GET() {
  try {
    ...
    const dataWithSerializableDates: AuditRecord[] = mockAdmissionDischargeRecords.map(record => ({
      ...record,
    ...
    console.error("API Error in /api/admissions-discharges:", error);
    const errorMessage = error instanceof Error ? error.message : String(error);
    return NextResponse.json(
        { message: "Internal Server Error fetching admission/discharge records.", error: errorMessage },
        { status: 500 }
    );
  }
}
