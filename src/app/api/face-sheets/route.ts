...
import { mockFaceSheetAuditRecords } from '@/lib/mock-data';
...
export async function GET() {
  try {
    ...
    const dataWithSerializableDates: AuditRecord[] = mockFaceSheetAuditRecords.map(record => ({
      ...record,
    ...
    console.error("API Error in /api/face-sheets:", error);
    const errorMessage = error instanceof Error ? error.message : String(error);
    return NextResponse.json(
        { message: "Internal Server Error fetching face sheet records.", error: errorMessage },
        { status: 500 }
    );
  }
}
