...
import { mockProgressNoteAuditRecords } from '@/lib/mock-data';
...
export async function GET() {
  try {
    ...
    const dataWithSerializableDates: AuditRecord[] = mockProgressNoteAuditRecords.map(record => ({
      ...record,
    ...
    console.error("API Error in /api/progress-notes:", error);
    const errorMessage = error instanceof Error ? error.message : String(error);
    return NextResponse.json(
        { message: "Internal Server Error fetching progress note records.", error: errorMessage },
        { status: 500 }
    );
  }
}
