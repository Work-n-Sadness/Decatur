...
import { mockMarRecords } from '@/lib/mock-data';
...
export async function GET() {
  try {
    ...
    const dataWithSerializableDates: AuditRecord[] = mockMarRecords.map(record => ({
      ...record,
    ...
    console.error("API Error in /api/mar-logs:", error);
    const errorMessage = error instanceof Error ? error.message : String(error);
    return NextResponse.json(
        { message: "Internal Server Error fetching MAR records.", error: errorMessage },
        { status: 500 }
    );
  }
}
